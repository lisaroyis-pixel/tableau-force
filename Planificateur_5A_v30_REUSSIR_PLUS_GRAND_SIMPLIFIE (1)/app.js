...
function renderReussir(){
  pageTitle.textContent='Les clés de ma réussite';
  state.reussir=state.reussir||{};
  const r=state.reussir;
  r.semester=r.semester||1;
  r.tab=r.tab||'classe';
  r.observations=Array.isArray(r.observations)?r.observations:[];

  const keys=[
    ['R','Rayonner en français'],
    ['É','Être fiable'],
    ['U','Utiliser les ressources'],
    ['S','Saisir les occasions d’agir'],
    ['S','Structurer mon matériel, mon temps et mon travail'],
    ['I','Interagir positivement avec les autres'],
    ['R','Réfléchir et m’autoréguler']
  ];

  const students=(state.class_students||'').split(/\n+/).map(x=>x.trim()).filter(Boolean);
  const semesterObs=r.observations.filter(o=>Number(o.semester||1)===Number(r.semester));

  content.innerHTML=`
    <div class="tf-reussir">
      <section class="reussir-intro">
        <div>
          <span class="eyebrow">2026–2027 · 5A</span>
          <h2>Les clés de ma RÉUSSITE</h2>
          <p>Je fais de mon mieux, j’apprends et je grandis.</p>
        </div>
        <div class="reussir-count"><strong>${semesterObs.length}</strong><span>observation${semesterObs.length===1?'':'s'}</span></div>
      </section>

      <div class="reussir-tabs">
        <button data-rtab="classe" class="${r.tab==='classe'?'active':''}">👥 Ma classe</button>
        <button data-rtab="historique" class="${r.tab==='historique'?'active':''}">🕘 Historique</button>
        <button data-rtab="sommaire" class="${r.tab==='sommaire'?'active':''}">📊 Sommaire HH</button>
        <div class="reussir-semesters">
          <button data-sem="1" class="${Number(r.semester)===1?'active':''}">1er semestre</button>
          <button data-sem="2" class="${Number(r.semester)===2?'active':''}">2e semestre</button>
        </div>
      </div>

      <section class="reussir-keys">
        ${keys.map(([letter,label])=>`
          <article class="reussir-key">
            <span class="reussir-letter">${letter}</span>
  ...
  const renderPanel=()=>{
    ...
    if(r.tab==='classe'){
      panel.innerHTML=`
        <section class="reussir-card">
          <div class="reussir-card-head">
            <div><strong>Ma classe</strong><small>Ajouter une observation rapidement</small></div>
            <button class="primary-btn" id="addReussirObs">+ Observation</button>
          </div>
          ${students.length?`
            <div class="reussir-students">
              ${students.map(n=>`<button class="reussir-student" data-student="${esc(n)}">${esc(n)}</button>`).join('')}
            </div>`:
            `<p class="muted-copy">Ajoute d’abord ta liste dans la section Élèves.</p>`}
        </section>`;
      const add=(student='')=>{
        const chosen=student || prompt('Nom de l’élève :','');
        if(!chosen)return;
        const existing=document.getElementById('reussirQuickAdd');
        if(existing) existing.remove();
        const box=document.createElement('div');
        box.id='reussirQuickAdd';
        box.className='reussir-quick-add';
        box.innerHTML=`
          <div class="quick-add-head"><strong>${esc(chosen)}</strong><button type="button" data-close-quick>×</button></div>
          <p>Appuie sur la clé observée :</p>
          <div class="quick-key-grid">
            ${keys.map(([letter,label],i)=>`<button type="button" data-quick-key="${i}"><b>${letter}</b><span>${esc(label)}</span></button>`).join('')}
          </div>
          <label class="quick-note">Note facultative <input id="quickReussirNote" placeholder="Petit commentaire…"></label>`;
        document.querySelector('.reussir-card').prepend(box);
        box.querySelector('[data-close-quick]').onclick=()=>box.remove();
        box.querySelectorAll('[data-quick-key]').forEach(btn=>btn.onclick=()=>{
          const [letter,label]=keys[Number(btn.dataset.quickKey)];
          const note=(document.getElementById('quickReussirNote').value||'').trim();
          r.observations.unshift({id:Date.now(),semester:Number(r.semester),student:chosen,key:letter,label,note,date:new Date().toISOString()});
          saveState(); renderReussir();
        });
      };
      document.getElementById('addReussirObs').onclick=()=>add('');
      document.querySelectorAll('[data-student]').forEach(b=>b.onclick=()=>add(b.dataset.student));
    } else if(r.tab==='historique'){
      panel.innerHTML=`
        <section class="reussir-card">
          <div class="reussir-card-head"><div><strong>Historique</strong><small>${r.semester===1?'1er':'2e'} semestre</small></div></div>
          <div class="reussir-history">
          ${semesterObs.length?semesterObs.map(o=>`
            <article>
              <div><strong>${esc(o.student||'Classe')}</strong><span class="key-chip">${esc(o.key||'')}</span></div>
              <p>${esc(o.note||'')}</p>
              <small>${new Date(o.date).toLocaleDateString('fr-CA')}</small>
              <button data-del-obs="${o.id}">Supprimer</button>
            </article>`).join(''):`<p class="muted-copy">Aucune observation pour ce semestre.</p>`}
          </div>
        </section>`;
      ...
      const counts={};
      semesterObs.forEach(o=>counts[o.key]=(counts[o.key]||0)+1);
      panel.innerHTML=`
        <section class="reussir-card">
          <div class="reussir-card-head"><div><strong>Sommaire HH</strong><small>Vue d’ensemble des observations</small></div></div>
          <div class="reussir-summary">
            ${keys.map(([letter,label],i)=>`
              <div><span>${letter}</span><strong>${esc(label)}</strong><b>${counts[letter]||0}</b></div>`).join('')}
          </div>
        </section>`;
    }
  };
  ...
  document.querySelectorAll('[data-rtab]').forEach(b=>b.onclick=()=>{
    r.tab=b.dataset.rtab;saveState();renderReussir();
  });
  document.querySelectorAll('[data-sem]').forEach(b=>b.onclick=()=>{
    r.semester=Number(b.dataset.sem);saveState();renderReussir();
  });
  renderPanel();
}
